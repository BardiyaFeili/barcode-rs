...
pub fn render(active_components: &mut [Component], mode: &Mode, focused_idx: usize, config: &Config) -> Result<(), Box<dyn Error>> {
    ...
    for component in &tiled_center {
        if active_comp_ptr.is_some_and(|ptr| std::ptr::eq(*component as *const Component, ptr)) {
            let (cx, cy) = component.window.visual_cursor.unwrap_or((component.cursor.x, component.cursor.y));
    ...
    for component in floating.iter_mut() {
         if active_comp_ptr.is_some_and(|ptr| std::ptr::eq(*component as *const Component, ptr)) {
            ...
            active_comp_info = Some((abs_x + border_offset, abs_y + border_offset, cx, cy, component.window.viewpoint));
        }
    }
...
fn draw(
    ...
    let focused_indices: Vec<bool> = tiled_center.iter()
        .map(|c| ctx.focused_comp_ptr.is_some_and(|ptr| std::ptr::eq(*c as *const _, ptr)))
        .collect();
    ...
    for (idx, component) in tiled_center.into_iter().enumerate() {
        ...
        for y in 0..win_h {
            ...
            if is_focused && ctx.config.editor.line_indicator {
                if let Some(info) = &component.window.visual_cursor_info {
                    if let Some(&(p_y, _, _, _)) = info.get(visual_idx) {
                        if p_y == component.cursor.y as usize {
            ...
            if gutter_w > 0 {
                if let Some(info) = &component.window.visual_cursor_info {
                    ...
                    if let Some(&(p_y, _, _, is_first)) = info.get(visual_idx) {
                        ...
                        let fg_color = if is_current { ctx.config.theme.gutter_active_fg } else { ctx.config.theme.gutter_fg };
                        
                        let g_bg = if is_current_line {
                            ctx.config.theme.line_indicator_bg.or(ctx.config.theme.gutter_bg)
                        } else {
                        ...
                        if let Some(bg_color) = if is_current_line { ctx.config.theme.line_indicator_bg.or(bg) } else { bg } {
                            queue!(stdout, SetBackgroundColor(bg_color))?;
                        } else {
                            queue!(stdout, SetBackgroundColor(Color::Reset))?;
                        }
            ...
            let available_row_w = (terminal_width - current_x).saturating_sub(gutter_w as u16) as usize;
            let row_w = (content_w as usize).saturating_sub(gutter_w).min(available_row_w);
            ...
            if let Some(line) = component.window.content.get(y as usize) {
                ...
                for (char_idx, c) in line.chars().enumerate() {
                    ...
                    if let Some(color) = line_highlights.and_then(|h| h.get(char_idx).copied().flatten()) {
            ...
            if idx < tiled_count - 1 && current_x + win_w - 1 < terminal_width {
                let border_color = if tiled_count == 2 {
                    ...
                    if focused_indices[idx] || (idx + 1 < focused_indices.len() && focused_indices[idx + 1]) {
                        theme.accent
                    } else {
                        theme.border
                    }
                ...
                queue!(stdout, cursor::MoveTo(current_x + win_w - 1, y), SetForegroundColor(border_color), Print("│"))?;
    ...
    if let Some(status) = status_line {
        ...
        if y + 1 < terminal_height {
            queue!(stdout, cursor::MoveTo(0, y + 1), terminal::Clear(terminal::ClearType::FromCursorDown))?;
        }
    }
    for component in floating {
        ...
        let is_focused = ctx.focused_comp_ptr.is_some_and(|ptr| std::ptr::eq(component as *const _, ptr));

        let bg = win.colors.content_bg.or(theme.bg);
        let fg = win.colors.content_fg.unwrap_or(theme.fg);
        let border_fg = win.colors.border_fg.unwrap_or(if is_focused { theme.accent } else { theme.border });
        ...
        if has_border {
            queue_box(&mut stdout, abs_x, abs_y, win.window_width, win.window_height, win.border_style, border_fg, border_bg, terminal_width, terminal_height)?;
        }
